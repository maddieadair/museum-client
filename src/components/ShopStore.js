    ...
    <div className="flex flex-col px-16 pb-24 gap-y-16 font-inter border-b">
      {giftItems.length > 0 ? (
        <div className="grid grid-cols-4 gap-x-12 gap-y-12">
          {giftItems.map(item => (
            <div key={item.gift_index} className="flex flex-col gap-y-4 border-[1px] border-stone-30">
              <div className="h-64 overflow-hidden">
                <img
                  className="object-cover object-center size-full hover:scale-125 transition-all duration-500"
              ...
              <div className="h-1/2 p-10 flex flex-col gap-y-4 items-center justify-center">
                <h4 className="font-bold text-xl text-center hover:text-cinnabar hover:underline duration-500 ease-in-out transition-all decoration-1 underline-offset-4">{item.gift_name}</h4>

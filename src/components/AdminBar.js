...
export default function AdminBar({title}) {
  ...
  return (
    <div className="h-16 w-full text-slate-700 flex flex-row justify-between items-center gap-x-4 font-inter p-14 border-b ">
      ...
      <div className={`${showMenu ? "shadow-md text-gray-500 w-64 bg-white rounded-2xl flex flex-col pt-4 top-24 right-8 absolute border" : "hidden"}`}>
        <div className="px-6 pb-4 break-words">
          <p className="font-bold text-gray-700">John Doe</p>
          <p className="">email@gmail.com</p>
        </div>
        <hr></hr>
        <Link to="/curator/account" className="px-6 py-3 hover:bg-gray-100">Profile</Link>
        <hr></hr>
